...
// ==== FUNGSI POINTER ==== 
void ubahPassword(User* user) {
    cout << "Masukkan password baru: ";
    cin >> user->password;
    cout << "Password berhasil diubah!\n";
}

void tampilkanUser(const User* user) {
    cout << "\n--- Info Akun ---\n";
...
// ==== FUNGSI SORTING ====
// Selection Sort untuk mengurutkan username secara ascending
void selectionSortUsername(User daftarUser[], int userCount) {
    for (int i = 0; i < userCount - 1; i++) {
        int minIndex = i;
        for (int j = i + 1; j < userCount; j++) {
            if (daftarUser[j].username < daftarUser[minIndex].username) {
                minIndex = j;
            }
        }
        swap(daftarUser[i], daftarUser[minIndex]);
    }
}

// Insertion Sort untuk mengurutkan nomor kamar secara descending
void insertionSortKamarDescending(Kamar daftarKamar[], int kamarCount) {
    for (int i = 1; i < kamarCount; i++) {
        Kamar key = daftarKamar[i];
        int j = i - 1;
        while (j >= 0 && daftarKamar[j].nomor < key.nomor) {
            daftarKamar[j + 1] = daftarKamar[j];
            j--;
        }
        daftarKamar[j + 1] = key;
    }
...
// Quick Sort untuk mengurutkan nomor kamar secara ascending
void quickSortKamar(Kamar daftarKamar[], int low, int high) {
    if (low >= high) return;
    int mid = low + (high - low) / 2;
    int pivot = daftarKamar[mid].nomor;
    int i = low, j = high;
    while (i <= j) {
        while (daftarKamar[i].nomor < pivot) i++;
        while (daftarKamar[j].nomor > pivot) j--;
        if (i <= j) {
            swap(daftarKamar[i], daftarKamar[j]);
            i++;
            j--;
        }
    }
    if (low < j) quickSortKamar(daftarKamar, low, j);
    if (i < high) quickSortKamar(daftarKamar, i, high);
}

// Fungsi untuk melihat daftar user
void lihatDaftarUser(User daftarUser[], int userCount) {
    // Urutkan username secara ascending sebelum ditampilkan
    selectionSortUsername(daftarUser, userCount);
    ...
    cout << "+-----------------+\n";
    for (int i = 0; i < userCount; i++) {
        cout << "| " << setw(15) << daftarUser[i].username << " |\n";
    }
    cout << "+-----------------+\n";
}
...
int main() {
    ...
    while (true) {
        ...
        while (isAdmin) {
            ...
            switch (pilihan) {
                case 1:
                    tambahKamar(daftarKamar, kamarCount);
                    break;
                case 2:
                    insertionSortKamarDescending(daftarKamar, kamarCount); // Sorting descending sebelum lihat
                    lihatKamar(daftarKamar, kamarCount);
                    break;
                case 3:
                    cout << "Masukkan Nomor Kamar: "; cin >> nomor;
                    ubahStatusKamar(daftarKamar, kamarCount, nomor);
                    break;
                case 4:
                    cout << "Masukkan Nomor Kamar: "; cin >> nomor;
                    hapusKamar(daftarKamar, kamarCount, nomor);
                    break;
                case 5:
                    lihatDaftarUser(daftarUser, userCount);  // Tampilkan daftar user
                    break;
                case 6:
                    isAdmin = 0;
        ...
        while (loginIndex != -1) {
            ...
            switch (pilihan) {
                case 1:
                    ...
                    bookingKamar(daftarKamar, kamarCount, nomor);
                    break;
                case 2:
                    quickSortKamar(daftarKamar, 0, kamarCount - 1); // Sorting ascending sebelum lihat tersedia
                    lihatKamarTersedia(daftarKamar, kamarCount);
                    break;
                case 3:
                    ubahPassword(&daftarUser[loginIndex]);
                    break;
                case 4:
                    tampilkanUser(&daftarUser[loginIndex]);
                    break;
                case 5:
                    loginIndex = -1;
